...
async fn two_devices_on_one_account_converge() {
    ...
    let dev_a = kanso_client::register(&base, "a@example.com", "hunter2")
        .await
        .unwrap();
    let dev_b = kanso_client::login(&base, "a@example.com", "hunter2")
        .await
        .unwrap();
    ...
    let note = a
        .create_note(&nb.id, "Hello", "from device A")
        .await
        .unwrap();
...
async fn accounts_are_isolated() {
    ...
    let user1 = kanso_client::register(&base, "u1@example.com", "pw")
        .await
        .unwrap();
    let user2 = kanso_client::register(&base, "u2@example.com", "pw")
        .await
        .unwrap();
    ...
    a.create_note(&nb.id, "secret", "user 1 only")
        .await
        .unwrap();
...
async fn blob_round_trip_and_isolation() {
    ...
    let u1 = kanso_client::register(&base, "blob1@example.com", "pw")
        .await
        .unwrap();
    let u2 = kanso_client::register(&base, "blob2@example.com", "pw")
        .await
        .unwrap();

    let data = b"the quick brown fox jumps over the lazy dog".to_vec();
    let hash = kanso_client::put_blob(&base, &u1.token, &data)
        .await
        .unwrap();

    // Owner round-trips the blob.
    assert_eq!(
        kanso_client::get_blob(&base, &u1.token, &hash)
            .await
            .unwrap(),
        Some(data.clone())
    );
    ...
    assert_eq!(
        kanso_client::get_blob(&base, &u2.token, &hash)
            .await
            .unwrap(),
        None
    );
...
async fn token_refresh_yields_a_working_token() {
    ...
    let auth = kanso_client::register(&base, "refresh@example.com", "pw")
        .await
        .unwrap();
